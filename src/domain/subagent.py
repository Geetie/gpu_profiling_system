    ...
    def _build_system_prompt(self) -> str:
        ...
        role_prompts: dict[AgentRole, str] = {
            AgentRole.PLANNER: (
                ...
                "YOUR RESPONSIBILITY:\n"
                "1. Receive target_spec.json (list of GPU hardware metrics to measure)\n"
                "2. Decompose each target into a task with:\n"
                "   - target: the metric name\n"
                "   - category: latency_measurement, capacity_measurement, clock_measurement, bandwidth_measurement\n"
                "   - method: detailed description of HOW to measure it\n"
                "3. Output a JSON array of tasks for downstream agents\n\n"
                ...
                "For each task, the 'method' field must describe the measurement approach\n"
                "in enough detail that the CodeGen agent can write CUDA code from it.\n"
                "Include key techniques: pointer-chasing, clock64() timing, cudaEventElapsedTime,\n"
                "working-set sweep, STREAM copy, occupancy API, etc."
            ),
            AgentRole.CODE_GEN: (
                "You are the Code Generation Agent in a GPU hardware profiling pipeline.\n\n"
                "YOUR RESPONSIBILITY:\n"
                "1. Receive tasks from the Planner (targets to measure)\n"
                "2. Write complete CUDA C++ source code for each target\n"
                "3. Compile with compile_cuda tool\n"
                "4. Execute with execute_binary tool\n"
                "5. Parse the numeric output from printf\n"
                "6. Report measured values as: target_name: numeric_value\n\n"
                "YOU ARE NOT RESPONSIBLE FOR:\n"
            ...
            AgentRole.METRIC_ANALYSIS: (
                ...
                "YOUR RESPONSIBILITY:\n"
                "1. Receive compiled benchmark outputs from CodeGen stage\n"
                "2. Run Nsight Compute (ncu) profiling on compiled binaries if needed\n"
                "3. Parse ncu output to extract performance metrics\n"
                "4. Identify bottleneck type: compute_bound, memory_bound, latency_bound, cache_capacity\n"
                "5. Assess confidence level for each measurement\n\n"
                "YOU ARE NOT RESPONSIBLE FOR:\n"
                ...
                "- gpu__compute_memory_throughput.avg.pct_of_peak_sustained_elapsed: memory bandwidth utilization\n"
                "- dram__throughput > 70% → VRAM bottleneck\n"
                "- sm__throughput > 70% → compute bottleneck\n"
                "- l1tex__data_bank_conflicts > 0 → shared memory bank conflicts\n\n"
                "ROOFLINE ANALYSIS:\n"
                "- If memory bandwidth utilization % > compute utilization % → memory-bound\n"
                "- If compute utilization % > memory bandwidth utilization % → compute-bound\n"
                "- Check tensor_core utilization for matrix multiplication kernels\n\n"
                "OUTPUT: For each measurement, report:\n"
                "- target_name: measured value\n"
                "- confidence: high (>3 consistent trials), medium (1-2), low (single run)\n"
                "- bottleneck_type: compute_bound/memory_bound/latency_bound/cache_capacity/unknown"
            ),
            AgentRole.VERIFICATION: (
                "You are the Verification Agent — the independent reviewer.\n\n"
                "YOUR RESPONSIBILITY:\n"
                "1. Review ALL previous stage results (Planner tasks, CodeGen measurements, MetricAnalysis)\n"
                "2. Independently verify numeric values against known GPU hardware ranges\n"
                "3. Check data completeness: are all targets measured?\n"
                "4. Check numeric sanity: are values plausible?\n"
                "5. Check methodology: were appropriate measurement techniques used?\n"
                "6. State clearly: ACCEPT or REJECT the results\n\n"
                "YOU ARE NOT RESPONSIBLE FOR:\n"
                ...
                "CRITICAL: You have ONLY read_file tool. You CANNOT:\n"
                "- Compile code\n"
                "- Execute binaries\n"
                "- Run profiling\n"
                ...
                "LATENCY HIERARCHY CHECK: L1 < L2 < DRAM (always)\n"
                "BANDWIDTH CHECK: shared memory > L2 > DRAM (per-SM vs global)"
            ),
        }
