...
_SM_THROUGHPUT = (
    ...
    "  #include <algorithm>\n"
    "  #include <cstring>\n\n"
    "⚠️ FOLLOW THE VERIFIED FORMULA IN YOUR SYSTEM PROMPT EXACTLY.\n"
    "The system prompt contains the step-by-step formula with code — use it.\n"
    "Key reminders:\n"
    "  - DOUBLE-precision FMA only (NOT float!)\n"
    "  - clock64() inside kernel for actual_freq_mhz (NOT cudaDevAttrClockRate!)\n"
    "  - peak_flops = sm_count * fp64_per_sm * actual_freq_mhz * 1e6 * 2\n"
    "  - HARDCODE iterations=10000000, grid=sm_count*4 blocks x 256 threads\n"
    "  - IGNORE --size/--repeats\n"
    "  - WARMUP before timed measurement\n"
    "  - volatile double* sink, #pragma unroll 1\n\n"
...
_GPU_COMPUTE_MEM_THROUGHPUT = (
    ...
    "The system prompt contains the step-by-step formula with code — use it.\n"
    "Key reminders:\n"
    "  - FUSED read-compute-write: val = input[i]; val = val*1.0001f+0.001f; output[i] = val;\n"
    "  - FMA chain MUST depend on value READ from memory (NOT register-only!)\n"
    "  - Register-only FMA = 0.09% throughput!\n"
    "  - const float* __restrict__ input, volatile float* output\n"
    "  - 64MB+ buffer (16M+ floats), sm_count*4 blocks x 256 threads\n"
    "  - HARDCODE n=16777216, grid=sm_count*4 (IGNORE --size/--repeats)\n"
    "  - WARMUP before timed measurement\n"
    "  - peak_bw = (mem_clock_khz/1000.0) * 1e6 * (bus_width_bits/8) * 2 / 1e9\n"
    "  - achieved_bw = (2.0 * buffer_size_bytes) / elapsed_seconds / 1e9\n"
    "  - pct = (achieved_bw / peak_bw) * 100.0\n\n"
