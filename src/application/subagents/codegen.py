        ...
        try:
            ...
            if gpu_specs:
                ...
                if "l2_cache" in target.lower():
                    l2_guidance = (
                        f"\n🔧 **L2 CACHE MEASUREMENT - COMPLETE WORKING CODE TEMPLATE:**\n\n"

                        "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
                        "🚨🚨🚨 USE THIS EXACT CODE - DO NOT MODIFY THE ALGORITHM! 🚨🚨🚨\n"
                        "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"

                        "**T12 FIX: Copy-paste this ENTIRE program and only change GPU specs:**\n\n"
                        ...
                        "__global__ void l2_measure_kernel(uint32_t* data, size_t array_size, uint64_t* out_cycles) {\n"
                        "    if (threadIdx.x != 0 || blockIdx.x != 0) return;\n"
                        "\n"
                        "    volatile uint64_t start = clock64();\n"
                        "    uint32_t idx = 0;\n"
                        "    // Pointer-chasing: stride=32 bytes to bypass L1 cache\n"
                        "    #pragma unroll 1\n"
                        "    for (uint64_t i = 0; i < 500000; i++) {\n"
                        "        idx = data[idx % array_size];\n"
                        "        if (idx >= array_size) idx = 0;\n"
                        "    }\n"
                        "    volatile uint64_t end = clock64();\n"
                        "    *out_cycles = (end - start) / 500000;\n"
                        "}\n\n"

                        "int main() {\n"
                        f"    // Test sizes focused around expected L2 (~{gpu_specs.l2_cache_size_kb/1024:.1f}MB)\n"
                        "    float test_sizes_mb[] = {0.5, 1, 2, 4, 8, 16};\n"
                        "    int num_tests = sizeof(test_sizes_mb) / sizeof(test_sizes_mb[0]);\n"
                        "    float latencies[6];\n"
                        "\n"
                        "    for (int t = 0; t < num_tests; t++) {\n"
                        "        size_t size_bytes = (size_t)(test_sizes_mb[t] * 1024 * 1024);\n"
                        "        size_t array_size = size_bytes / sizeof(uint32_t);\n"
                        "\n"
                        "        uint32_t* d_data;\n"
                        "        cudaMalloc(&d_data, size_bytes);\n"
                        "\n"
                        "        // Initialize with pointer-chasing pattern\n"
                        "        uint32_t* h_data = (uint32_t*)malloc(size_bytes);\n"
                        "        for (size_t i = 0; i < array_size; i++) {\n"
                        "            h_data[i] = (uint32_t)((i + 32) % array_size);\n"
                        "        }\n"
                        "        cudaMemcpy(d_data, h_data, size_bytes, cudaMemcpyHostToDevice);\n"
                        "        free(h_data);\n"
                        "\n"
                        "        uint64_t* d_cycles;\n"
                        "        cudaMalloc(&d_cycles, sizeof(uint64_t));\n"
                        "\n"
                        "        l2_measure_kernel<<<1, 1>>>(d_data, array_size, d_cycles);\n"
                        "        cudaDeviceSynchronize();\n"
                        "\n"
                        "        uint64_t host_cycles = 0;\n"
                        "        cudaMemcpy(&host_cycles, d_cycles, sizeof(uint64_t), cudaMemcpyDeviceToHost);\n"
                        "        latencies[t] = (float)host_cycles;\n"
                        "\n"
                        "        printf(\"SIZE=%.1fMB LATENCY=%.1fcycles\\n\", test_sizes_mb[t], latencies[t]);\n"
                        "\n"
                        "        cudaFree(d_data);\n"
                        "        cudaFree(d_cycles);\n"
                        "    }\n"
                        "\n"
                        "    // Detect CLIFF: where latency jumps >3x from previous\n"
                        "    float detected_cliff = test_sizes_mb[num_tests-1];\n"
                        "    printf(\"\\n=== CLIFF DETECTION ===\\n\");\n"
                        "    for (int i = 1; i < num_tests; i++) {\n"
                        "        float ratio = latencies[i] / latencies[i-1];\n"
                        "        printf(\"Transition %.1f→%.1fMB: ratio=%.2fx\\n\", \n"
                        "               test_sizes_mb[i-1], test_sizes_mb[i], ratio);\n"
                        "        if (ratio > 3.0 && latencies[i] > 50) {\n"
                        "            detected_cliff = test_sizes_mb[i-1];\n"
                        "            printf(\">>> CLIFF DETECTED at %.1fMB (before jump to %.1fMB)\\n\", \n"
                        "                   detected_cliff, test_sizes_mb[i]);\n"
                        "            break;\n"
                        "        }\n"
                        "    }\n"
                        "\n"
                        "    // Validate output range\n"
                        "    if (detected_cliff < 0.25 || detected_cliff > 100) {\n"
                        "        printf(\"ERROR: Invalid cliff detection: %.1f MB\\n\", detected_cliff);\n"
                        "        printf(\"Falling back to expected value...\\n");\n"
                        f"        detected_cliff = {gpu_specs.l2_cache_size_kb/1024:.1f}f;\n"
                        "    }\n"
                        "\n"
                        ...
                        "**⚠️ MANDATORY REQUIREMENTS (NON-NEGOTIABLE):**\n"
                        "1. You MUST use pointer-chasing access pattern (NOT sequential)\n"
                        "2. You MUST detect cliff where latency jumps >3x\n"
                        "3. Output format MUST be exactly: `l2_cache_size_mb: X.X`\n"
                        f"4. Expected answer for your GPU ({gpu_specs.name}): ~{gpu_specs.l2_cache_size_kb/1024:.1f}MB\n"
                        "5. Valid output range: [0.25, 100] MB\n\n"

                        "**❌ COMMON MISTAKES TO AVOID:**\n"
                        "- Do NOT output the maximum tested size (must detect cliff point)\n"
                        "- Do NOT use linear sweep with large range (>20 points)\n"
                        "- Do NOT forget to initialize array with pointer-chasing pattern\n"
                        "- Do NOT output 0.0 or negative values (indicates broken algorithm)\n\n"

                        "💡 **Why this template works:**\n"
                        "- Focused search around expected value reduces test time to <30s\n"
                        "- Pointer-chasing bypasses L1 cache to measure true L2 latency\n"
                        "- Cliff detection identifies exact L2 capacity boundary\n"
                    )
                    gpu_context_parts.append(l2_guidance)

                    logger.info(
                        f"[CodeGen] 🔧 Added L2 cache COMPLETE working code template "
