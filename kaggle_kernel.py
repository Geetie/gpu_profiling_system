...
def run_pipeline(project_root: str, target_spec_path: str) -> bool:
    ...
    try:
        from src.infrastructure.probing.fallback_config import (
            FALLBACK_PROBES_ENABLED, COMPLIANCE_MODE,
        )
        print(f"  Fallback Probes: {'ENABLED (DEBUG)' if FALLBACK_PROBES_ENABLED else 'DISABLED (STRICT)'}")
        print(f"  Compliance Mode: {COMPLIANCE_MODE.upper()}")
        if not FALLBACK_PROBES_ENABLED:
            print("  ✅ All CUDA code will be generated by LLM (spec.md §5.1 compliant)")
            print("  ✅ Eligible for full 'Engineering Reasoning' score (30/30 points)")
        else:
            print("  ⚠️  WARNING: Fallback mode enabled - results may be penalized")
            print("  ⚠️  Expected scoring impact: -25 to -30 points")
    except ImportError:
        print("  (fallback_config not available - using defaults)")
        print("  Assuming STRICT mode for compliance")
    print("=" * 60 + "\n")
